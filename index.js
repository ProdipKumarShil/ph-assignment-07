...
startQuiz.addEventListener("click", () => {
  ...
  let x = setInterval(() => {
    if (counterNum < 0) {
      countDown.classList.remove("flex");    // ****** maybe countDown spelling error
...
const loadQuiz = async () => {
  ...
  const data = await res.json();                   // ******* may here should be an arrow function 
  quizData = data;
  displayQuiz(data);
};
...
const displayQuiz = (data) => {
  ...
  data.forEach((quiz, i) => {           // ******* maybe quiz.question 
...
document.querySelector("#submit").addEventListener("click", () => {
  ...
  if (storage) {
    localStorage.setItem(
      "results",
      JSON.stringify([            // ********** maybe object not properly set
        ...storage,
        {
          marks: totalMark,
